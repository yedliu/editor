    ...
    if (Array.isArray(value) || value instanceof Set) {
      ...
      (value as any[]).forEach((subValue, index) => {
        ...
        if (!this.options.enableCircularCheck || !this.isCircular(subValue)) {
          let realTargetType;
          var subdiscriminator: Discriminator = null;
          var subkeepDiscriminatorProperty = false;
          ...
          const newType =
            targetType && typeof targetType !== 'function'
              ? targetType.typeFunction(options)
              : undefined;
          if (
            ...
            subdiscriminator = targetType.options.discriminator;
            keepDiscriminatorProperty =
              targetType.options.keepDiscriminatorProperty;
          } else if (
            newType != null &&
            this.options &&
            this.options.typeMaps &&
            this.options.typeMaps.length > 0 &&
            this.options.typeMaps.some(x => x.type == newType)
          ) {
            var option = this.options.typeMaps.find(x => x.type == newType);
            subdiscriminator = option?.discriminator;
            subkeepDiscriminatorProperty = option?.keepDiscriminatorProperty;
            if (!subdiscriminator.property || !subdiscriminator.subTypes) {
              subdiscriminator = null;
            }
          }
          if (subdiscriminator) {
            if (this.transformationType === TransformationType.PLAIN_TO_CLASS) {
              realTargetType = subdiscriminator.subTypes.find(
                subType => subType.name === subValue[subdiscriminator.property],
                // subValue && Reflect.has(subValue, subdiscriminator.property) &&
              );

              realTargetType === undefined
                ? (realTargetType = newType)
                : (realTargetType = realTargetType.value);
              if (!subkeepDiscriminatorProperty)
                delete subValue[subdiscriminator.property];
            }
            ...
            if (
              ...
              !option?.keepDiscriminatorProperty
            ) {
              var subtype = subdiscriminator.subTypes.find(
                subType => subType.value === subValue.constructor,
              );
              if (subtype) {
                subValue[subdiscriminator.property] = subtype.name;
              }
            }
          } else {
            // if (subValue?.constructor && subValue.constructor != Object && targetType instanceof Function
            //   && MetaHelper.getAncestors(subValue.constructor).includes(targetType)) {
            //   realTargetType = subValue.constructor
      ...
      for (let key of keys) {
        ...
        if (targetType && isMap) {
          ...
          if (metadata) {
            ...
            var keepDiscriminatorProperty = false;
            if (
              metadata.options &&
              ...
              discriminator = metadata.options.discriminator;
              keepDiscriminatorProperty =
              ...
              keepDiscriminatorProperty = option?.keepDiscriminatorProperty;
              ...
              if (!(value[valueKey] instanceof Array)) {
                if (
                  ...
                  type = discriminator.subTypes.find(subType => {
                    if (subValue && discriminator.property in subValue) {
                      return subType.name === subValue[discriminator.property];
                    }
                  });
                  type === undefined ? (type = newType) : (type = type.value);
                  if (!keepDiscriminatorProperty) {
                    if (subValue && discriminator.property in subValue) {
                ...
                type = metadata ? metadata : newType;
              }
            } else {
              type = newType;
            }
        ...
        if (!this.options.enableCircularCheck || !this.isCircular(subValue)) {
          ...
          if (newValue instanceof Map) {
            ...
            Reflect.set(newValue, newValueKey, finalValue);
          ...
          if (newValue instanceof Map) {
            newValue.set(key, finalValue);
          } else if (finalValue != undefined) {
            Reflect.set(newValue, key, finalValue);
            //newValue[newValueKey] = finalValue;
          }
        }
      }
