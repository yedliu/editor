...
export interface Props {
  ...
  onLoad?: (sender: FreeScrollbar) => void;
}
...
export default class FreeScrollbar extends React.PureComponent<Props, State> {
  ...
  public scrollbarScrollThrottle =
    this.props.onScrollbarScrollTimeout > 0
      ? this.throttle(
          ...
          this.props.onScrollbarScrollTimeout,
        )
      : this.props.onScrollbarScroll;
  ...
  private updateTrackVisibilities = () => {
    ...
    var targetHandlerPos = this.getHandlerPos();
    if (
      scrollHeight === this.lastScrollHeight &&
      scrollWidth === this.lastScrollWidth &&
      JSON.stringify(targetHandlerPos) === JSON.stringify(this.state.handlerPos)
    )
      return;
    this.setState({
      showVeriticalTrack:
        (this.el.firstElementChild as HTMLElement).offsetHeight >
        this.offsetHeight,
      showHorizontalTrack:
        (this.el.firstElementChild as HTMLElement).offsetWidth >
        this.offsetWidth,
      handlerPos: targetHandlerPos,
    });
  ...
  getHandlerPos() {
    let el = this.el;
    var scrollHeight = (this.el.firstElementChild as HTMLElement).offsetHeight;
    var scrollWidth = (this.el.firstElementChild as HTMLElement).offsetWidth;
    let top =
      (el.scrollTop / (scrollHeight - this.offsetHeight)) *
      (1 - this.offsetHeight / scrollHeight) *
      100;
    let bottom = (1 - this.offsetHeight / scrollHeight) * 100 - top;
    if (bottom < 0) bottom = 0;
    let left =
      (el.scrollLeft / (scrollWidth - this.offsetWidth)) *
      (1 - this.offsetWidth / scrollWidth) *
      100;
    let right = (1 - this.offsetWidth / scrollWidth) * 100 - left;
    if (right < 0) right = 0;
    let pos = {
      top: top,
      bottom: bottom,
  ...
  private resetHandlerHider = () => {
    if (this.props.autohide) {
      ...
      this.handlerHider = window.setTimeout(() => {
  ...
  private handlerContainerScroll = () => {
    ...
    this.setState({ handlerPos: this.getHandlerPos() });
  ...
  private handleHandlerMouseMove = (event: MouseEvent) => {
    if (this.activeHandler === Direction.Vertical) {
      ...
      this.el.scrollTop =
        this.lastContainerScrollTop +
        (delY / this.offsetHeight) * this.lastScrollHeight;
    }
    if (this.activeHandler === Direction.Horizontal) {
      let delX = event.clientX - this.lastMousePos.left;
      this.el.scrollLeft =
        this.lastContainerScrollLeft +
        (delX / this.offsetWidth) * this.lastScrollWidth;
    }
  };
  ...
  public render() {
    ...
    let verticalHandlerStyles: React.CSSProperties = {
      top: this.state.handlerPos.top + '%',
      bottom: 100 - verticalHandlerHeight - this.state.handlerPos.top + '%',
      opacity: this.state.hideHandler ? 0 : 1,
    };
    let horizontalHandlerStyles: React.CSSProperties = {
          ...
          <div
            className={`FreeScrollbar-vertical-track ${
              this.props.className
                ? this.props.className + '-vertical-track'
                : ''
            }`}
            style={
              this.props.className
          ...
          >
            <div
              className={`FreeScrollbar-vertical-handler ${
                this.props.className
                  ? this.props.className + '-vertical-handler'
                  : ''
              }`}
              onMouseDown={event => {
        ...
        {this.state.showHorizontalTrack ? (
          <div
            className={`FreeScrollbar-horizontal-track ${
              this.props.className
                ? this.props.className + '-horizontal-track'
                : ''
            }`}
            style={
              this.props.className
          ...
          >
            <div
              className={`FreeScrollbar-horizontal-handler ${
                this.props.className
                  ? this.props.className + '-horizontal-handler'
                  : ''
              }`}
              onMouseDown={event => {
